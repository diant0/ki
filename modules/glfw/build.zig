...
fn generateWaylandHeaders(b: *std.Build, cache_subpath: []const u8) !void {
    ...
    while (try protocols_dir_iterator.next()) |protocol_dir_entry| {
        ...
        const exit_code_client_header = blk: {
            const client_header_abspath = try std.fmt.bufPrint(&output_file_path_buf, "{s}/{s}/{s}-client-protocol.h", .{ b.cache_root.path orelse return error.CacheRootIsCwd, cache_subpath, protocol_name });
        ...
        const exit_code_private_code = blk: {
            const private_code_abspath = try std.fmt.bufPrint(&output_file_path_buf, "{s}/{s}/{s}-client-protocol-code.h", .{ b.cache_root.path.?, cache_subpath, protocol_name });
...
fn updateGamepadMappings(self: *std.Build.Step, _: std.Build.Step.MakeOptions) !void {
    const b = self.owner;
