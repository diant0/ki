...
pub fn stbttFromMemAlloc(allocator: std.mem.Allocator, bytes: []const u8, parameters: Parameters) !@This() {
...

    {
        ...
        for (charset, 0..) |codepoint, i| {
...

            {
                ...
                stbtt.stbtt_GetCodepointBitmapBox(&info, codepoint_c_int, scale, scale, &coords[0], &coords[1], &coords[2], &coords[3]);
    ...
    for (charset, 0..) |codepoint, i| {
        ...
        stbtt.stbtt_MakeCodepointBitmap(&info, &alpha_component_buffer[byte_offset], glyphs[i].pixel_rect[2], glyphs[i].pixel_rect[3], atlas_pixel_size[0], scale, scale, codepoint_c_int);
    }
