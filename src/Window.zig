...
pub fn initAlloc(self: *@This(), allocator: std.mem.Allocator, init_options: @This().InitOptions) !void {
    ...
    const handle = glfw.glfwCreateWindow(@intCast(size[0]), @intCast(size[1]), init_options.title.ptr, if (init_options.fullscreen) monitor else null, null) orelse return error.GLFWCreateWindowFailed;
...
fn __onResize(handle: ?*glfw.GLFWwindow, width: c_int, height: c_int) callconv(.C) void {
    ...
    window.event_queue.pushBack(.{
        ...
        log.print(.Error, "could not push to window's event queue on resize event: {s}\n", .{@errorName(e)});
    };
}
...
fn __onKeyboard(handle: ?*glfw.GLFWwindow, glfw_key: c_int, _: c_int, action: c_int, _: c_int) callconv(.C) void {
    ...
    window.event_queue.pushBack(.{ .key = .{
        .key = key,
        .down = down,
        .repeat = repeat,
    } }) catch |e| {
        log.print(.Error, "could not push to window's event queue on key event: {s}\n", .{@errorName(e)});
    };
}
...
fn __onChar(handle: ?*glfw.GLFWwindow, glfw_char: c_uint) callconv(.C) void {
    ...
    window.event_queue.pushBack(.{
        ...
        log.print(.Error, "could not push to window's event queue on char event: {s}\n", .{@errorName(e)});
    };
}
...
fn __onCursorMove(handle: ?*glfw.GLFWwindow, x: f64, y: f64) callconv(.C) void {
    ...
    window.event_queue.pushBack(.{
        ...
        log.print(.Error, "could not push to window's event queue on cursor move event: {s}\n", .{@errorName(e)});
    };
}
...
fn __onCursorButton(handle: ?*glfw.GLFWwindow, glfw_button: c_int, action: c_int, _: c_int) callconv(.C) void {
    ...
    window.event_queue.pushBack(.{ .cursor_button = .{
        .button = button,
        .down = down,
    } }) catch |e| {
        log.print(.Error, "could not push to window's event queue on cursor button event: {s}\n", .{@errorName(e)});
    };
}
...
fn __onScroll(handle: ?*glfw.GLFWwindow, x: f64, y: f64) callconv(.C) void {
    ...
    window.event_queue.pushBack(.{
        ...
        log.print(.Error, "could not push to window's event queue on scroll event: {s}\n", .{@errorName(e)});
    };
}
...
fn __onFocus(handle: ?*glfw.GLFWwindow, focused_int: c_int) callconv(.C) void {
    ...
    window.event_queue.pushBack(.{
        ...
        log.print(.Error, "could not push to window's event queue on focus event: {s}\n", .{@errorName(e)});
    };
}
...
fn __onTerminationRequest(handle: ?*glfw.GLFWwindow) callconv(.C) void {
    ...
    window.event_queue.pushBack(.{
        ...
        log.print(.Error, "could not push to window's event queue on termination request event: {s}\n", .{@errorName(e)});
    };
}
